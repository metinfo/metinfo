      ...
      success: function(result) {
        ...
        <div class="media-body ">
         ...
         ${item.appname}
          ...
          <div class="card-text">${item.info}</div>
        </div>
        </a>
        </div>
        ${
        ...
        row.off().on('click','.link:not([target])',function() {
          ...
          if (url && url !== 'javascript:;') {
            if (version) {
              M.is_admin?(window.location.href =url):$('.btn-pageset-common-page').attr({'data-url':url.replace(M.url.admin+'#/',''),title:$('h5',this).text()}).trigger('clicks');
            } else {
