...
(function() {
  ...
  function delAll() {
    $(document).on('click', `#seo-link-table .btn-delete-all`, function() {
      let ids = []
      const $ids = that.obj.find("#seo-link-table [name='id']:checked")
      if ($ids.length === 0) {
  ...
  function renderEditModal(modal) {
    Object.keys(that.activeData).map(item => {
      ...
      if (item === 'show_ok' || item === 'com_ok' || item === 'nofollow') {
        parseInt(that.activeData[item])?modal.find(`#${item}`).attr('checked', 'checked'):modal.find(`#${item}`).removeAttr('data-checked');
        return
      }
      if (item === 'module') {
