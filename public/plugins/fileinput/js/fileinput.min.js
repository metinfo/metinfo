...
(function() {
	...
	M.component.modal_options['.img-library-modal'] = {
		...
		modalHeaderappend: '<button type="button" class="btn btn-warning float-right invisible btn-img-folder-back"><i class="fa-reply mr-2"></i>'+METLANG.back_folder_list+'</button>',
		modalSize: 'xl',
		modalFullheight: 1,
	...
	$.fn.extend({
		...
		metFileInput: function() {
			...
			$(this).each(function(index, el) {
				...
				// 验证初始化
				required && metui.use('form', function() {
	...
	function fileThumbnail(imgurl) {
		var imgurl_handle=imgurl.toLowerCase();
			file_html = imgurl_handle.indexOf('.png') >= 0 || imgurl_handle.indexOf('.jpeg') >= 0 || imgurl_handle.indexOf('.jpg') >= 0 || imgurl_handle.indexOf('.bmp') >= 0 || imgurl_handle.indexOf('.gif') >= 0 || imgurl_handle.indexOf('.ico') >= 0 ? '<img src="' + imgurl + '" class="file-preview-image vertical-align-middle"></a>' : '<div class="file-preview-other text-grey" style="width:68px;"><i class="glyphicon glyphicon-file"></i></div>',
	...
	$(document).on('show.bs.modal', '.img-library-modal', function(event) {
		...
		$('.img-library-modal .btn-img-folder-back').addClass('invisible');
		metui.ajax({
			url: filelist_url
		}, function(result) {
			if (result) {
				...
				$.each(result, function(index, val) {
					html += '<li class="text-center my-4 col-6 col-md-4 col-lg-3 col-xl-2">' + '<a href="javascript:;" title="' + val.name + '" data-path="' + val.path + '" class="d-block text-content dir">' + '<i class="fa-folder-open-o h1"></i>' + '<h4 class="h6 mb-0">' + val.name + '</h4>' + '</a>' + '</li>';
				});
	...
	// 图片库-选择文件夹
	$(document).on('dblclick', '.img-library-modal .img-folder-list li .dir,.img-library-modal .img-library-list li a.dir,.img-library-modal .btn-img-folder-back,.img-library-modal .img-library-breadcrumb a', function(event) {
		...
		$loader.removeClass('hide');
		$('.img-library-modal .btn-img-folder-back').removeClass('invisible');
		metui.ajax({
			url: filelist_url,
			data: {
				dir: dir
	...
	// 图片库-选择图片
	$(document).on('click', '.img-library-modal .img-library-list li .img', function(event) {
